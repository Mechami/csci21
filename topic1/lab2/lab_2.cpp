...
void MakeChange(int initial_value, int &quarters, int &dimes, int &nickels,
                int &pennies) {
  quarters = static_cast<int>(floor(initial_value / 25));
  initial_value -= (quarters * 25);
  dimes = static_cast<int>(floor(initial_value / 10));
  initial_value -= (dimes * 10);
  nickels = static_cast<int>(floor(initial_value / 5));
  initial_value -= (nickels * 5);
